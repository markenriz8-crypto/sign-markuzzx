...
// Rate limiter (UNLIMITED - commented out for unlimited access)
// const limiter = rateLimit({
//   windowMs: Number(process.env.RATE_LIMIT_DURATION || 60) * 1000,
//   max: Number(process.env.RATE_LIMIT_POINTS || 60),
...
// Apply rate limiting (DISABLED for unlimited access)
// app.use(limiter);
...
app.all("/sign", async (req, res) => {
  try {
    ...
    log.info({ url, signature }, "Signed URL");
    return res.json(out);
  } catch (err) {
